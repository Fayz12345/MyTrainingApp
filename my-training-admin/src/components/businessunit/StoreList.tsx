...
const StoreList: React.FC<StoreListProps> = ({ refreshTrigger }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Get current user ID to filter stores for BusinessUnit users
      ...
      // Filter stores: BusinessUnit users only see stores they created
      let filteredStores = storesResult.data as Store[];
      if (userId) {
        filteredStores = filteredStores.filter(store => store.createdBy === userId);
      }
