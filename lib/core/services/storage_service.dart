...
class StorageService {
  ...
  /// Gets a pre-signed URL for a PDF file from S3
  /// This generates a complete URL with authentication that expires after ~15 minutes
  static Future<String> getPdfUrl(String pdfKey) async {
    try {
      final result = await Amplify.Storage.getUrl(
        path: StoragePath.fromString(pdfKey),
      );
      final url = (await result.result).url.toString();
      safePrint('📄 PDF URL generated for key: $pdfKey');
      safePrint('📄 Complete URL: $url');
      return url;
    } catch (e) {
      safePrint('Error getting PDF URL: $e');
      rethrow;
    }
  }
