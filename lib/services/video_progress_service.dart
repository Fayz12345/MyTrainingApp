...
class VideoProgressService {
  ...
  /// Save video watch position for a course
  static Future<void> saveVideoProgress(String courseId, Duration position,
    ...
    try {
      ...
      if (duration != null && duration.inMilliseconds > 0) {
        ...
        safePrint(
            '[VIDEO_PROGRESS_SERVICE] ✅ Saved progress: Key="$courseId", Position=${position.inSeconds}s (${position.inMinutes}m ${position.inSeconds % 60}s), Duration=${duration.inSeconds}s (${duration.inMinutes}m ${duration.inSeconds % 60}s)');
      } else {
        safePrint(
            '[VIDEO_PROGRESS_SERVICE] ✅ Saved progress: Key="$courseId", Position=${position.inSeconds}s (${position.inMinutes}m ${position.inSeconds % 60}s), Milliseconds=${position.inMilliseconds}ms');
      }
    } catch (e) {
  ...
  /// Get saved video watch position for a course
  static Future<Duration?> getVideoProgress(String courseId) async {
    try {
      ...
      if (milliseconds != null) {
        final duration = Duration(milliseconds: milliseconds);
        safePrint(
            '[VIDEO_PROGRESS_SERVICE] ✅ Loaded progress: Key="$courseId", Position=${duration.inSeconds}s (${duration.inMinutes}m ${duration.inSeconds % 60}s), Milliseconds=${milliseconds}ms');
        return duration;
      }
      safePrint(
          '[VIDEO_PROGRESS_SERVICE] ⚠️ No saved progress found for Key="$courseId"');
      return null;
    } catch (e) {
  ...
  static Future<void> markVideoCompleted(String courseId) async {
    try {
      ...
      print(
          '>>> VideoProgressService: Marked video as completed for course $courseId');
    } catch (e) {
  ...
  static Future<bool> isVideoCompleted(String courseId) async {
    try {
      ...
      print(
          '>>> VideoProgressService: Video completed status for course $courseId: $completed');
      return completed;
    } catch (e) {
    ...
    try {
      ...
      final percentage =
          (savedPosition.inMilliseconds / totalDuration.inMilliseconds)
              .clamp(0.0, 1.0);
      print(
          '>>> VideoProgressService: Progress percentage for course $courseId: ${(percentage * 100).toStringAsFixed(1)}%');
      return percentage;
    } catch (e) {
      print(
          '>>> VideoProgressService: Error calculating progress percentage: $e');
      return 0.0;
    }
  }

  /// Clear video progress (for resetting)
  static Future<void> clearVideoProgress(String courseId) async {
    try {
      ...
      print('>>> VideoProgressService: Cleared progress for course $courseId');
    } catch (e) {
