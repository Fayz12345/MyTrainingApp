...
// lkogin amnd  reg.
  static Future<void> signIn({
    ...
    try {
      ...
      if (result.isSignedIn) {
        ...
        // CRITICAL FIX: Wait for credentials to be stored before proceeding
        // This prevents "CredentialStoreLoadingStoredCredentials is not a subtype of CredentialStoreSuccess" error
        safePrint(
            '[LOGIN_FLOW] [AUTH_SERVICE] Waiting for credentials to be stored...');
        await Future.delayed(const Duration(milliseconds: 500));

        // Retry mechanism to ensure credentials are ready
        int retries = 0;
        const maxRetries = 3;
        while (retries < maxRetries) {
          try {
            // Try to fetch session to verify credentials are stored
            await Amplify.Auth.fetchAuthSession();
            safePrint(
                '[LOGIN_FLOW] [AUTH_SERVICE] ✅ Credentials stored and ready');
            break;
          } catch (e) {
            retries++;
            if (retries >= maxRetries) {
              safePrint(
                  '[LOGIN_FLOW] [AUTH_SERVICE] ⚠️ Credentials not ready after $maxRetries retries, but sign-in was successful');
              // Don't throw - sign-in was successful, credentials will be ready soon
            } else {
              safePrint(
                  '[LOGIN_FLOW] [AUTH_SERVICE] ⚠️ Credentials not ready yet, retrying... ($retries/$maxRetries)');
      ...
      // Handle credential store errors specifically
      if (e.toString().contains('CredentialStoreLoadingStoredCredentials') ||
          e.toString().contains('not a subtype')) {
        safePrint(
            '[LOGIN_FLOW] [AUTH_SERVICE] ⚠️ Credential store loading error - this is usually temporary');
        // Wait a bit and retry once
        await Future.delayed(const Duration(milliseconds: 1000));
        try {
          await Amplify.Auth.fetchAuthSession();
          safePrint(
              '[LOGIN_FLOW] [AUTH_SERVICE] ✅ Retry successful after credential store error');
          return; // Sign-in was actually successful
        } catch (retryError) {
          safePrint(
              '[LOGIN_FLOW] [AUTH_SERVICE] ❌ Retry also failed: $retryError');
          throw Exception(
              'Sign in completed but credentials are still loading. Please try again in a moment.');
        }
      }
  ...
  static Future<void> signInWithGoogle() async {
    ...
    try {
      ...
      safePrint(
          '[LOGIN_FLOW] [AUTH_SERVICE] Checking Amplify configuration...');
      /*
      try {
        final authPlugin = Amplify.Auth.getPlugin(AmplifyAuthCognito.pluginKey);
        safePrint(
            '[LOGIN_FLOW] [AUTH_SERVICE] ✅ Auth plugin found: ${authPlugin.runtimeType}');
      } catch (e) {
        safePrint('[LOGIN_FLOW] [AUTH_SERVICE] ❌ Auth plugin not found: $e');
        throw Exception(
            'Amplify Auth is not properly configured. Please restart the app.');
      }
     */

      safePrint(
          '[LOGIN_FLOW] [AUTH_SERVICE] Calling Amplify.Auth.signInWithWebUI() with Google provider...');
      safePrint('[LOGIN_FLOW] [AUTH_SERVICE] Provider: AuthProvider.google');

      final result = await Amplify.Auth.signInWithWebUI(
        provider: AuthProvider.google,
      );

      safePrint('[LOGIN_FLOW] [AUTH_SERVICE] ✅ Google login success');
      safePrint('[LOGIN_FLOW] [AUTH_SERVICE] Result: $result');
      safePrint(
          '[LOGIN_FLOW] [AUTH_SERVICE] Is signed in: ${result.isSignedIn}');

      // Log success to file
      await FileLogger.log(
        'GOOGLE_SIGNIN_SUCCESS',
        variables: {
          'is_signed_in': result.isSignedIn.toString(),
      ...
      if (errorString.contains('UserCancelledException') ||
          errorString.contains('UserCancelled') ||
          errorString.contains('cancelled')) {
        safePrint('[LOGIN_FLOW] [AUTH_SERVICE] User cancelled sign-in');
        throw Exception('Sign in was cancelled.');
      } else if (errorString.contains('NetworkException') ||
          errorString.contains('network') ||
          errorString.contains('Network')) {
        safePrint('[LOGIN_FLOW] [AUTH_SERVICE] Network error detected');
        throw Exception(
            'Network error. Please check your internet connection and try again.');
      } else if (errorString.contains('InvalidAccountTypeException') ||
          errorString.contains('No user pool registered') ||
          errorString.contains('user pool')) {
        safePrint(
            '[LOGIN_FLOW] [AUTH_SERVICE] Configuration error: No user pool');
        throw Exception(
            'GOOGLE_LOGIN_NOT_AVAILABLE: OAuth configuration is missing. '
            'Please check amplify_outputs.json and ensure OAuth is properly configured.');
      } else if (errorString.contains('PlatformException') ||
          errorString.contains('Platform')) {
        safePrint('[LOGIN_FLOW] [AUTH_SERVICE] Platform-specific error');
        throw Exception('Platform error occurred. Please ensure:\n'
            '1. Android: Intent filter is configured in AndroidManifest.xml\n'
            '2. iOS: URL scheme is configured in Info.plist\n'
            '3. Both use: com.mytrainingapp://');
      } else {
        safePrint('[LOGIN_FLOW] [AUTH_SERVICE] Unknown error type');
        throw Exception('Google sign-in failed: ${e.toString()}\n\n'
            'Please run diagnostics from the test screen to identify the issue.');
      }
    }
  }
