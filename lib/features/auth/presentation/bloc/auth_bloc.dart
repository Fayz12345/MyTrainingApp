...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onCheckUserGroups(
    ...
    try {
      ...
      try {
        // CRITICAL FIX: Add retry logic for credential store loading errors
        int retries = 0;
        const maxRetries = 3;
        AuthUser? user;
        ...
        while (retries < maxRetries) {
          try {
            ...
            // Check if it's a credential store loading error
            if ((e
                        .toString()
                        .contains('CredentialStoreLoadingStoredCredentials') ||
                    e.toString().contains('not a subtype')) &&
                retries < maxRetries - 1) {
              retries++;
              safePrint(
                  '[LOGIN_FLOW] [STEP 2.2] ⚠️ Credential store loading, retrying... ($retries/$maxRetries)');
              await Future.delayed(Duration(milliseconds: 500 * retries));
              continue;
            } else {
              // Not a credential store error, or max retries reached
              rethrow;
            }
          }
        }
        ...
        if (user == null) {
          throw Exception('Failed to get current user after retries');
        }
      } catch (e) {
  ...
  Future<void> _onSignIn(
    ...
    // Clear any previous error state when starting new login attempt
    emit(const AuthUnauthenticated());
    // Don't emit AuthLoading - let UI handle loading state on button
    ...
    try {
      ...
      // After successful sign in, check user groups (this will emit AuthLoading, but that's OK for group check)
      ...
      if (errorString.contains('Sign in requires additional steps') ||
          errorString.contains('UserNotConfirmedException') ||
          errorString.contains('verification')) {
        safePrint('[LOGIN_FLOW] [SIGNIN] Email verification required');
  ...
  Future<void> _onSignUp(
    ...
    // Clear any previous error state when starting new signup attempt
    emit(const AuthUnauthenticated());
    // Don't emit AuthLoading - let UI handle loading state on button
    // API call happens in background
    try {
      ...
      // If signup requires confirmation, emit that state
 ...
 */
  ...
  Future<void> _onConfirmSignUp(
    ...
    try {
      ...
      // Log user login activity (after signup confirmation)
      ActivityLogger.logUserLogin(
        loginMethod: 'email',
        success: true,
      );
  ...
  Future<void> _onSignInWithGoogle(
    ...
    try {
      ...
      // Log failed login attempt (unless user cancelled)
